...
class FollowAppSettingsController extends BaseController {
  ...
  List<FollowUser> buildAutoCleanPool() {
    ...
    final followedIds = followList
        ...
        .toSet(); // set性能略优
    final followedHistories = histories
        .where((history) => followedIds.contains(history.id))
        .toList();
    ...
    // 根据动态条件筛选出需要清理的 关注id
    final df = dynamicFilter(
      followedHistories,
      conditions,
      takeLast: takeLast.value,
    );
    final uidsToClean = df.map((history) => history.id).toSet();

    final autoCleanPool = followList
        .where((follow) => uidsToClean.contains(follow.id))
        .toList();
